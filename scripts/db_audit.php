...
class DatabaseAuditor {
    ...
    private function testEloquent() {
        $this->results['eloquent']['tests'] = [];
        
        try {
            // Test 1: DB Facade connection
            if (class_exists('\Illuminate\Support\Facades\DB')) {
                $result = \Illuminate\Support\Facades\DB::select('SELECT 1 as test');
                $this->results['eloquent']['tests']['db_facade'] = [
                    'status' => 'pass',
                    'result' => 'DB facade query successful'
                ];
            } else {
                $this->results['eloquent']['tests']['db_facade'] = [
                    'status' => 'skip',
                    'reason' => 'DB facade class not found'
                ];
            }
            
            // Test 2: Schema Facade
            if (class_exists('\Illuminate\Support\Facades\Schema')) {
                $hasTable = \Illuminate\Support\Facades\Schema::hasTable('admin_users');
                $this->results['eloquent']['tests']['schema_facade'] = [
                    'status' => 'pass',
                    'result' => 'Schema facade functional',
                    'admin_users_exists' => $hasTable
                ];
            } else {
                $this->results['eloquent']['tests']['schema_facade'] = [
                    'status' => 'skip',
                    'reason' => 'Schema facade class not found'
                ];
            }
            
            // Test 3: Query Builder
            if (class_exists('\Illuminate\Database\Capsule\Manager')) {
                $count = \Illuminate\Database\Capsule\Manager::table('settings')->count();
                $this->results['eloquent']['tests']['query_builder'] = [
                    'status' => 'pass',
                    'result' => 'Query builder functional',
                    'settings_count' => $count
                ];
            }
            
            // Test 4: Model query (if AdminUser model exists)
            if (class_exists('\App\Models\AdminUser')) {
                $count = \App\Models\AdminUser::count();
                $this->results['eloquent']['tests']['model_query'] = [
                    'status' => 'pass',
                    'result' => 'Model queries functional',
                    'admin_users_count' => $count
                ];
            } else {
                $this->results['eloquent']['tests']['model_query'] = [
                    'status' => 'skip',
                    'reason' => 'AdminUser model not found'
                ];
            }
            
            $this->results['eloquent']['status'] = 'operational';
            
        } catch (\Exception $e) {
            $this->results['eloquent']['status'] = 'error';
            $this->results['eloquent']['test_error'] = $e->getMessage();
            $this->results['errors'][] = 'Eloquent tests failed: ' . $e->getMessage();
        }
    }
